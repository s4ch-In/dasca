...
export class DetailsComponent implements OnInit {
  ...
  printRec(print: boolean) {

    let mb = JSON.parse(localStorage.getItem('recData'))
    if (mb.ground) {
      mb = Object.assign({}, mb.ground, mb)
    } else {
      mb = Object.assign({}, mb.user, mb)
    }

    localStorage.setItem('formData', JSON.stringify(mb))
    ...
    if (confirm('Are you want to really print form')) {
      ...
      this.service.api(this.globals.newReceipt, this.newReceiptForm.value).subscribe(res => {
        if (res.s) {
          ...
          this.userId = this.newReceiptForm.value.userId
          this.category = this.newReceiptForm.value.category
          this.newReceiptForm.reset();
          this.newReceiptForm.get('userId').setValue(this.userId)
          this.newReceiptForm.get('category').setValue(this.category)
          this.newReceiptForm.get('mode').setValue('CASH')
          this.newReceiptForm.get('sport').setValue('Cricket')
          this.newReceiptForm.get('membership').get('q1').get('FY').setValue(this.fyq1)
          this.newReceiptForm.get('membership').get('q2').get('FY').setValue(this.fyq2)
          this.newReceiptForm.get('membership').get('q3').get('FY').setValue(this.fyq3)
    ...
    if (confirm('Are you want to really print form')) {
      ...
      this.service.api(this.globals.payb, this.paymentForm.value).subscribe(res => {
        if (res.s) {
          ...
          this.paymentForm.get('mode').setValue('CASH')
